...
Feature selection callbacks can be called from different stages of the
feature selection process. The stages are prefixed with `on_*`.
...
- on_eval_after_xs:
  ...
  `ObjectiveFSelectAsync$eval()`. The functions must have two arguments
...
- on_eval_after_resample:
  ...
  (`function()`)  
  Stage called after a feature subset is evaluated. Called in
  `ObjectiveFSelectAsync$eval()`. The functions must have two arguments
  named `callback` and `context`. The `resample_result` is available in
  the `context`.
...
- on_eval_before_archive:
  ...
  Called in `ObjectiveFSelectAsync$eval()`. The functions must have two
...
- on_fselect_result_begin:
  ...
  `FSelectInstance*$assign_result()`. The functions must have two
...
(`$state`), e.g. settings that affect the callback itself. Feature
selection callbacks access
[ContextAsyncFSelect](https://mlr3fselect.mlr-org.com/dev/reference/ContextAsyncFSelect.md)
and
